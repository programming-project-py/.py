# Call the function used in the code  
import pandas as pd    
import jdatetime 
import ast
import os
...
# Receive payment amount on a certain date, store and organize according to categories 
# and subcategories and attribute the amounts to the relevant units in the csv file.
# Time : Enter the date and time of arrival of transaction values and convert the 
# Gregorian date to solar using the jdatetime module.  
def cost_service():
...
# Separate the function delet date befor a specified interval , or save previous 
# information and add new payment information to the original csv file.
def pay_service():
...
# Definition of a function whith the ability to determine how payments are calculated 
# based on the number of people , area of units and other situations.
...
# Calculate the ratio of amounts paid according to the number of people        
        if s==1:    
...
# Calculate the ratio of amounts paid in terms of unit area      
        elif s==2:
            while p<(len(ast.literal_eval(D2.loc[N,'name']))):
                    d3.loc[p,'name']=(ast.literal_eval(D2.loc[N,'name']))[p] 
                    d3.loc[p,'date']=D2.loc[N,'date']
...
# Calculate payment amounts in equal proportions between related units      
        elif s==3:
            while p<(len(ast.literal_eval(D2.loc[N,'name']))):
                    d3.loc[p,'name']=(ast.literal_eval(D2.loc[N,'name']))[p] 
                    d3.loc[p,'date']=D2.loc[N,'date']
...
# Definition of a function to calculate the financial for each unit on the 
# specified date of adding the financial balance column to the final output.        
def get_report():
...
# Functional definition for faster access to code output dataforms 
# by calculation and reporting method.        
def service_chose():
